...
public class SearchController {
    ...
    public String index(@QueryParam("filter") @DefaultValue("") final String filter, @CookieParam(FavoritesCookie.NAME) final Cookie favoriteCookie) {
    ...
    private List<LatestCountyStatistic> fetchStatisticsBasedOnFilter(final String filter, final List<String> favorites) {
        switch (filter) {
        case "all":
            return statisticsProvider.loadAll();
        case "":
            return List.of();
        default:
            return statisticsProvider.searchWithFilterExcludingFavorites(filter, favorites);
        }
    }
